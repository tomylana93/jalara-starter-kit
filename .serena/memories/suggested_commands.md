...
- Two-tier gate composed from one script per CI job, so the local command runs
  ...
  `ci:e2e` are the rest. `composer run ci:check` (fast) = `ci:static`,
...
- `.github/workflows/tests.yml` runs the gate as four parallel jobs — `static`,
  ...
  sources import gitignored Wayfinder modules that only Artisan generates. The
  workflow-level `RUN_FULL` expression picks the tier and is duplicated into the
  `e2e` and `installer` job-level `if`s, which cannot read `env`; `pest` selects
  coverage through a step-level `if` instead. The `pull_request`
  `converted_to_draft` activity triggers the fast tier and shares the PR
  concurrency group, cancelling a superseded full run. The public installer is
  an independently running parallel full-tier job used to minimize workflow wall
  time.
