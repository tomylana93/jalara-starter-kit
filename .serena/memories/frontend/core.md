...
- Page layout selection is centralized in `resources/js/app.ts`: Welcome has no layout, `auth/*` uses AuthLayout, `account/*` nests AppLayout + AccountLayout (`resources/js/layouts/account/Layout.vue`), other pages use AppLayout. The `settings/*` page/layout namespace was retired; user account management (profile/security/appearance) lives under `account/*`.
