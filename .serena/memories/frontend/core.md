...
- Page layout selection is centralized in `resources/js/app.ts`: Welcome has no layout, `auth/*` uses AuthLayout, `account/*` nests AppLayout + AccountLayout (`resources/js/layouts/account/Layout.vue`), and all other pages (including `settings/*`) use AppLayout. Settings pages use `PageWrapper` within each page instead of a nested settings layout. `account/*` is per-user account management (profile/security/appearance); `settings/*` is application-wide administration.
