...
- Use Serena for project navigation, symbol discovery, reference analysis,
  precise refactoring, and project memory. Prefer symbolic reads over full-file
  reads when the target is known.
...
- Use Context7 for current non-Laravel library documentation and shadcn tools
  for registry-backed UI components when those tools are available.
...
- Activate the smallest set whose trigger metadata matches the task, read each
  selected skill completely, and load only the referenced resources needed.
...
4. Run focused tests first, then the required format, lint, type, or build
   checks for the affected surface.
5. Review the diff and update Serena memory only when the task revealed stable,
